...
# Uso del Bullpen: Si el lanzador abridor ha permitido más de 3 carreras en las últimas 2 entradas, considerar cambiar al relevista.
def bullpen_usage_rule(game_state):
    if game_state['runs_allowed_last_2_innings'] > 3:
        return True
    return False

# Cambio de Bateador: Si el equipo está perdiendo por 1 carrera en la última entrada y hay un bateador con un promedio de bateo bajo, considerar un bateador emergente.
...
# Estrategia de Bateo y Carrera: Si hay un corredor en primera base y menos de 2 outs, considerar la estrategia de bateo y carrera.
def hit_and_run_rule(game_state):
    if game_state['runner_on_first'] and game_state['outs'] < 2:
        return True
    return False

# Defensa en el Infield: Si hay un corredor en tercera base y menos de 2 outs, jugar con la defensa del infield adentro para prevenir una carrera
def infield_in_rule(game_state):
    if game_state['runner_on_third'] and game_state['outs'] < 2:
        return True
    return False

# Cambio de Posición Defensiva: Si el bateador contrario tiene una alta tendencia a batear hacia el lado derecho del campo, ajustar la defensa hacia ese lado.
